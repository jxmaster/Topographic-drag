...
!#if 0
...
#else
                !set 2+1 for continuous dimension
                call infld('OA', ncid_topoGWD,'lon','nvar_dirOA','lat',1,pcols,1,nvar_dirOA,begchunk, &
                                endchunk,  oadir(:,:,:),  found, grid_map='PHYS')
                terroutchar(1)='terr'
                terroutchar(2)='terrx'
                ...
                terroutchar(4)='wt'
                do i=1,4
                call infld(terroutchar(i),ncid_topoGWD,'lon','indexb','lat',1,pcols,1,indexb,begchunk,endchunk,terrout(i,:,:,:),found,grid_map='PHYS')
enddo
#endif
...
!
        !===========Jinbo Xie=========
        integer , intent(in) :: gwd_ls,gwd_bl
	...
	!=================================================================
      call gwdo2d(dudt=rublten(ims,kms),dvdt=rvblten(ims,kms)                  &
             ,dthdt=rthblten(ims,kms)                                          &
              ...
              ,gsd_gwd_ls=gwd_ls,gsd_gwd_bl=gwd_bl)
                !=============Jinbo Xie 
...
!=====Jinbo Xie==================
   ...
   real(r8)                 :: taufb(its:ite,kts:kte+1)
        !===========Jinbo Xie=========
        integer , intent(in) :: gsd_gwd_ls,gsd_gwd_bl
        !===========Jinbo Xie=========
...
!Jinbo Xie
!do i = its,ite
!write(iulog,*)"Jinbo Xie kbl(i),klowtop(i),kpblmax",kbl(i),klowtop(i),kpblmax
!enddo
!Jinbo Xie
!
...
if (taub(i).gt.1) then
write(iulog,*) "Jinbo Xie xlinv(i), roll(i), ulow(i),ulow(i)*ulow(i)*ulow(i),gfobnv,efact,var(i),taub(i) ",xlinv(i), roll(i), ulow(i),ulow(i)*ulow(i)*ulow(i),gfobnv,efact,var(i),taub(i)
write(iulog,*)"Jinbo Xie gfobnv,gmax,tem,(tem + cg)*bnv(i),cg,bnv(i) ",gfobnv,gmax,tem,(tem + cg)*bnv(i),cg,bnv(i)
endif
...
!write(iulog,*)"Jinbo Xie maxval(taub),maxval(taufb)", maxval(taub),maxval(taufb)
